...
from ultralytics.nn.extra_modules.block import C2f_FFCM, RGCSPELAN, RepNCSPELAN4, CSP_MutilScaleEdgeInformationSelect,C2f_DCMB
...
def replace_c2f_with_c2f_v2(module):
    ...
    for name, child_module in module.named_children():
        if isinstance(child_module, C2f_DCMB):
            # Replace C2f with C2f_v2 while preserving its parameters
            shortcut = infer_shortcut(child_module.m[0])
            c2f_v2 = C2f_DCMB_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
                            n=len(child_module.m), shortcut=shortcut,
                            g=1,
                            e=child_module.c / child_module.cv2.conv.out_channels)
            transfer_weights_c2f_to_c2f_v2(child_module, c2f_v2)
    ...
    # for name, child_module in module.named_children():
    #     if isinstance(child_module, C2f_FFCM):
    #         # Replace C2f with C2f_v2 while preserving its parameters
    #         shortcut = infer_shortcut(child_module.m[0])
    #         c2f_v2 = C2f_FFCM_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
    #                         n=len(child_module.m), shortcut=shortcut,
    #                         g=1,
    #                         e=child_module.c / child_module.cv2.conv.out_channels)
    #         transfer_weights_c2f_to_c2f_v2(child_module, c2f_v2)
    ...
    # for name, child_module in module.named_children():
    #     if isinstance(child_module, RGCSPELAN):
    #         # Replace C2f with C2f_v2 while preserving its parameters
    #         c2f_v2 = RGCSPELAN_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
    #                         n=len(child_module.m), 
    #                         scale=child_module.mid / child_module.cv4.conv.out_channels ,
    #                         e=child_module.c / child_module.cv2.conv.out_channels)
    ...
    # for name, child_module in module.named_children():
    #     if isinstance(child_module, CSP_MutilScaleEdgeInformationSelect):
    #         # Replace C2f with C2f_v2 while preserving its parameters
    #         shortcut = infer_shortcut(child_module.m[0])
    #         c2f_v2 = CSP_MutilScaleEdgeInformationSelect_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
    #                         n=len(child_module.m), shortcut=shortcut,
    #                         g=1,
    #                         e=child_module.c / child_module.cv2.conv.out_channels)
    #         transfer_weights_c2f_to_c2f_v2(child_module, c2f_v2)
