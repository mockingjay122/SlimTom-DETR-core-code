...
from ultralytics.nn.extra_modules.block import IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, \
    ...
    RepConvN, SPDConv, CSPOmniKernel, DynamicAlignFusion, DWConv, EdgeEnhancer, MutilScaleEdgeInformationSelect,DynamicIncMixerBlock,ELA_HSFPN
from ultralytics.nn.extra_modules.attention import DualDomainSelectionMechanism
from ultralytics.nn.extra_modules.prune_module import RepNCSPELAN4_v2,C2f_DCMB_v2
...
def get_pruner(opt, model, example_inputs):
    ...
    # ultralytics/cfg/models/rt-detr/rtdetr-C2f-DIMB-ELA-HSFPN+POLA.yaml
    customized_pruners[RepConv] = RepConvPruner()
    for k, m in model.named_modules():
        ...
        if isinstance(m, RTDETRDecoder):
            ignored_layers.append(m)
        if isinstance(m, C2f_DCMB_v2):
            ignored_layers.append(m)
        if isinstance(m, ELA_HSFPN):
            ignored_layers.append(m.conv1x1)
